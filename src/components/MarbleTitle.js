...
// Entra entero tras una máscara (no por líneas: cada línea llevaría su propio trozo de mármol y
// saltaría al juntarse). Visible por defecto: solo se esconde en el instante en que arranca.
export default function MarbleTitle({ as: Tag = 'h2', id, className = '', children, start = 'top 85%' }) {
  ...
  useEffect(() => {
    ...
    const st = ScrollTrigger.create({
      ...
      once: true,
      onEnter: () => {
        tween = gsap.fromTo(el, { yPercent: 105 }, { yPercent: 0, duration: D.enter, ease: EASE, clearProps: 'transform' });
      },
    });
  ...
  return (
    <Tag id={id} className={`mask ${className}`}>
      <span ref={innerRef} className="mask__inner marble-text">{children}</span>
    </Tag>
  );
}
